...
{
	/**
	 * CurlResponse short summary.
	 *
	 * CurlResponse description.
	 *
	 * @version 1.0
	 * 
	 */
	...
	class CurlResponse
	{
		...
		public function __construct($ch, $response)
		{
			...
			if (curl_errno($ch) == CURLE_OK && $this->getHttpCode() >= 200  && $this->getHttpCode() <= 299)
			{
				$this->success = true;
			}
			else
			{
				$this->success = false;
				$curlError = new CurlError(curl_errno($ch), curl_error($ch));
