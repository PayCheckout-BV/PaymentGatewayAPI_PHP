...
{
	$response = new \PaymentGateway\Api\Gateway\PaymentJobs\PostResponse();
	$execute->getMappedResponse($response);
	$payjob = $response->getData();
	$jobref = $payjob->getReference();
	$payref = $payjob->getPayments()[0]->getReference();

	// Execute the get payment request
	$execute->callEndPoint("/api/gateway/payment-jobs/" .$jobref . "/payments/" . $payref . "?locale=NL_nl","GET");
	if ($execute->isSuccess())
	{
		// echo "<br/>job=" . $jobref . "payref= " . $payref . "json->" . ($execute->getResponseBody()) . "<br/>";

		$response = new \PaymentGateway\Api\Gateway\PaymentJobs\PaymentJobReference\Payments\PaymentReference\GetResponse();
		$execute->getMappedResponse($response);
