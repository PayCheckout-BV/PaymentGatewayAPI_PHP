...
$postRequest->addParameter(\PaymentGateway\Api\Gateway\PaymentJobs\PaymentJobParameter::simulatedStatus,"PAID");
...
if ($execute->isSuccess())
{
	...
	$refund = new \PaymentGateway\Api\Gateway\PaymentJobs\PaymentJobReference\Payments\PaymentReference\Refunds\PostRequest();
	$refund->setAmountToRefund(0.85);
	$refund->setCreditNumber("1234");
	$refund->setCreditNote("Credit note");

	//echo '<br/>refund json->' . json_encode($refund) . '<br/>';

	$execute->callEndPoint("/api/gateway/payment-jobs/" . $paymentjobRef . "/payments/" . $paymentRef . "/refunds?locale=NL_nl","POST",$refund);
	if ($execute->isSuccess())
	{
		// echo '<br/>response json->' . ($execute->getResponseBody()) . '<br/>';

		$result = $execute->getMappedResponse(new \PaymentGateway\Api\Gateway\PaymentJobs\PaymentJobReference\Payments\PaymentReference\Refunds\PostResponse());
